// failed attempt of changing the involvement of tt
...
{
	pthread_mutex_lock(&data->task[4]);
	if (time_after - time_before >= data->ttd)
		exit(1);
	pthread_mutex_unlock(&data->task[4]);
}
...
void lock_stick(t_data *data, int left_fork, int right_fork, int time)
{
	if (left_fork % 2 != 0)
	{
...
void unlock_stick(t_data *data, int left_fork, int right_fork, int time)
{
	if (left_fork % 2 != 0)
	{
...
void *philosopher (void *phi)
{
	...
	while (1)
	{
		...
		lock_stick(data, left_fork, right_fork, time);
		...
		time = data->tt;
		pthread_mutex_unlock(&data->task[3]);
		unlock_stick(data, left_fork, right_fork,time);
